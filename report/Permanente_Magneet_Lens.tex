...
\section{Criteria voor het lensontwerp}\label{ch:lensontwerp}
...
\section{Simulatie van de lenseigenschappen}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{SEM beelden van een raster waarbij het EMAT logo wordt gevormd door rasterbalken.}
...
\section{Kwantitatieve validatie focale lengte}
...
De opstelling die men toepast wordt getoond op Figuur \ref{fig:rear_mesh_shadowgraph}. In de methode beschreven in \cite{meshshadowgraphs} wordt er een puntbron op de optische as gevormd die zich op een afstand $z_0>f$ van de lens gevindt. Dit wordt de \textit{object-puntbron} genoemd. Als men de lens als dun veronderstelt, dan kan de dunne lensformule toegepast worden:
...
\begin{equation}
    \frac{e}{z_i+b}=\frac{E}{z_i+c+d}\Leftrightarrow\frac{E}{e}=M=1+\frac{d}{z_i+b}
\end{equation}
...
In het experiment dat in deze thesis uitgevoerd zal worden, maken we een reeks metingen van $z_i$ voor verschillende waarden van $z_0$. In de microscoop kan dit gerelaiseerd worden door de \textit{werkingsafstand} aan te passen. Deze instelling geeft de verticale afstand van de elektronenbron tot het punt waar de elektronenbundel geconvergeerd wordt. De werkingsafstand komt met andere woorden overeen met $z_0$.\\
...
Met deze simulaties is het mogelijk om de nauwkeurigheid van de rear mesh shadowgraph methode te vallideren. Het is mogelijk om op basis van het gesimuleerde detectorbeeld de magnificatie van het schaduwbeeld in te schatten en in te vullen in \ref{eq:image_dist_real}. Als we hieruit de focale lengte berekenen, dan vinden we een waarde die binnen $\pm 0.1$~mm van de werkelijke gesimuleerde waarde ligt. Dit is een indicatie dat de methode voldoende nauwkeurig is om de focale lengte te berekenen.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{Images/ThorLabs_cage.png}
...
Op basis van deze overwegingen werden een lenshouder, een rasterhouder en een apertuur ontworpen in Blender en geprint in een commercieel beschikbaar geleidend PLA \cite{hyuduo_conductive_pla} met een BambuLab A1 printer. De bijhorende \texttt{.step}-bestanden zijn samen met de broncode beschikbaar op GitHub. Figuur \ref{fig:3D_prints_measurements} toont de vervaardigde opstellingsonderdelen.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Images/3D_prints_measurements.jpeg}
...
In deze sectie bespreken we de uitvoering en de resultaten van het experiment. De foutenvlaggen die bij de waarden worden vermeld volgen uit een foutenpropagatie die wordt uitgewerkt in appendix \ref{ch:foutenpropagatie}. Figuur \ref{fig:tower} toont de kolom die in dit experiment gebruikt werd met de 3D-geprinte lens- en rasterhouders. De kolom wordt op de detector, recht onder de elektronenbron geplaatst. Figuur \ref{fig:tower_schematic} geeft een schematische illustratie de uitvoering van het experiment. 
...
De afstanden $z_{lens}$, $z_{raster}$ en $z_{detector}$ moeten nauwkeurig worden opgemeten om een nauwkeurige waarde voor de focale lengte te verkrijgen. De resultaten worden hieronder weergegeven:
...
Voor deze waarden werd er een reeks beelden opgenomen voor werkingsafstanden van 

Op elk beeld worden er zo veel mogelijk afstanden van de rasteropeningen opgemeten via een interactief Python-script waarbij de gebruiker de hoeken van een rasteropening kan aanklikken. Dit wordt getoond op Figuur \ref{fig:measure_pitch_distances}. Deze waarden worden uitgemiddeld. Met de berekende reeks waarden voor $z_i$ kan dan voor elke $z_0$ waarde de focale lengte berekend, waarna een uitmiddeling de finale waarde voor $f$ levert. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Images/measure_pitch_distances.png}
    \caption{Er wordt gebruik gemaakt van een Python-script waarmee de hoekpunten van de rasteropeningen kunnen worden aangeklikt, waarmee de magnificatie van het beeld berekend kan worden om uiteindelijk de focale lengte te vinden.}
