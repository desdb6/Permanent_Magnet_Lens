...
\end{equation}
We beschouwen nu enkel paraxiale elektronen, m.a.w. elektronen die zich dicht bij de optische as van het systeem ($r=0$) bevinden. Men kan aantonen dat $B_z(r, z)$ ontwikkeld kan worden in een reeks van machten in $r$. Stellen we $B(z)=B_z(r=0, z)$:
\begin{equation}\label{eq:b_z_expansion}
    B_z(r, z)=B(z)-\frac{r^2}{4}\frac{d^2B(z)}{dz^2}+\frac{r^4}{64}\frac{d^4B(z)}{dz^4}+O(r^6)
\end{equation}
Voor kleine $r$ benaderen we $B_z(r, z)$ door de eerste term in deze ontwikkemling, m.a.w. $B_z(r, z)=B(z)$. Nu is het mogelijk om een relatie tussen $B$ en $B_r$ af te leiden. Men beschouwt een cilindrisch volume rond de z-as van het systeem zoals op Figuur \ref{fig:maxwell_2_cilinder} en gebruiken de tweede Maxwellvergelijking in integraalvorm, welke ons vertelt dat de totale flux doorheen een gesloten ondervlak gelijk moet zijn aan $0$.
\begin{figure}[H]
    \centering
...
\begin{itemize}
    ...
    \item De asymptotische focale lengte is het punt waar de asymptoot voor $z\rightarrow\infty$ aan $g$ de optische as kruist. Als we een parallelle bundel op de lens insturen, lijkt het voor een observator die achter van het midden van de lens staat alsof de elektronen niet uit het reëel, maar het asymptotisch brandpunt komen.
\end{itemize}
...
\begin{enumerate}
    \item De waarde van focale lengte moet zo gekozen worden zodat de lens compatibel is met de beschikbare ruimte in een elektronenmicroscoop. Concreet zal deze lens getest worden in een Tescan MIRA SEM, waarin een maximale totale afstand van $228.4\mathrm{mm}$ van elektronenbron tot detector beschikbaar is. Om een bruikbare lens te maken is het daarom gewenst dat de focale lengte niet veel langer dan $10\mathrm{mm}$ is. De versnelspanning bedraagt $30$~keV, wat een voorwaarde legt op de minimale sterkte van het magnetisch veld. \\
...
\begin{figure}[H]
    ...
    \caption{Interactieve plot van $B(z)$ in functie van de lensparameters. Deze functie kan worden gebruikt om de afhankelijkheid van de verschillende parameters te bestuderen, en de lensparameters te optimaliseren om een gewenst magnetisch veld te krijgen. In dit geval werden de parameters $R_1=0.8\mathrm{mm}$, $R_2=3.25\mathrm{mm}$, en $d_g=0.8\mathrm{mm}$ gekozen. De $B_r$ instelling zorgt voor een herschaling van de $\mathrm{BH}$-curve.}
...
\end{equation}
De oplossing van vergelijking \ref{eq:paraxialevgl_rel_H54} kan voor dit veld worden uitgedrukt met goniometrische functies \cite{PWHawkes}. Het magnetisch veld kan ook benaderd worden door een constant veldzoals getoond op Figuur \ref{fig:fields_comparison_b}. Het voorschrift wordt gegeven door:
...
\end{figure}
We zien dat de piek van het Glaser's field een goede benadering is voor het veld van de lens, maar dit veld wijst altijd in dezelfde zin en is overal positief. Dit geeft een te grote lenswerking weg ver weg van het lenscentrum en geeft aanleiding tot een dikke lenswerking. Deze analytische oplossingsmethoden blijken onvoldoende nauwkeurig voor onze doelainden. Om accurate voorspellingen van onze lens te kunnen maken, kiezen we er daarom voor om de paraxiale beweginsvergelijking numeriek op te lossen en hieruit de lenseigenschappen te bepalen. Dit laat ons ook toe om later Monte Carlo simulaties van experimentele opstellingen uit te voeren, zodat we kunnen inschatten welke opstellingen ons bruikbare resultaten kunnen geven.\\

\subsubsection{Numerieke oplossing}
Om de paraxiale bewegingsvergelijking numeriek op te lossen, gebruiken we de solve\_ivp functie van de scipy.integrate bibliotheek. Figuur \ref{fig:setup_1} toont een plot van een numerieke oplossing en de berekende lenseigenschappen. De lenseigenschappen worden bepaald in de volgende stappen:
\begin{enumerate}
    \item Het objectvlak $z_0$ kan ingesteld worden op een positie ver van het lenscentrum weg. De paraxiale bewegingsvergelijking wordt voor de bijhorende beginvoorwaarden (zie \ref{ch:BVW}) opgelost met een 8ste orde Runge-Kutta methode (DOP853) om de twee lineair onafhankelijke oplossingen $g(z)$ en $h(z)$ te berekenen.
...
Met de analystische afschatting van het veld en de analytische afgeleiden zijn is het mogelijk om de asymptotische aberratiecoëfficiënten in vergelijkingen \ref{eq:distortion} en \ref{eq:chr_ab} van de lens te berekenen. De uitdrukkingen van deze coëfficiënten worden hieronder gertoond \cite{PWHawkes}.
\begin{align}
...
\end{align}
Merk op dat de oplossingen $g(z)$ en $h(z)$ van vergelijking \ref{eq:gh} optreden. Deze oplossingen moeten hierbij als dimensieloos beschouwd worden. Deze integralen zijn zeer zelden analytisch oplosbaar, en ook aangezien we geen anaylitische uitdrukkingen voor $g(z)$ en $h(z)$ hebben moeten we deze numeriek oplossen door de Riemann-som met een kleine $\Delta z$ te nemen. 
...
\begin{enumerate}
    ...
    \item De lens moet geschikt zijn om in de MIRA microscoop kunnen gebruiken. Concreet betekent dit dat de focale lengte die rond de $5-10\mathrm{mm}$ moet liggen. Hiervoor moet $\sigma_M$ voldoende groot zijn, en moet $R_1$ voldoende klein zijn zodat de ladingen zich niet te ver van de optische as bevinden.
    
    \item Het is gewenst dat de aberratiecoëfficiënten $D$ en $C_M$ in de buurt van typische waarden liggen om vervormde beelden te voorkomen. Typische waarden voor $D$ en $C_M$ zijn respectievelijk $\sim0.5\mathrm{mm}^{-2}$ en $\sim1-3$ \cite{Abbas_2016, JEOL_Aberrations}. De coëfficienten moeten in het ideale geval onder deze waarden blijven.

    \item Figuur \ref{fig:operating_point} toonde de $\mathrm{BH}$-curve van graad N35 NdFeB. Het werkingspunt mag de de elleboog van deze curve niet overschrijden, omdat het remanent veld van de magneet dan snel zou afnemen. Echter, de elleboog bevindt zich ongeveer bij $B_r=1.1\mathrm{T}, \mu_0 H_d = 1.2\mathrm{T}$, dus bij $\mu_0 H_d>B_r$. Het is met andere woorden niet mogelijk om de magneet (bij kamertemperatuur) in een circuit te demagnetiseren zonder een extern magnetisch veld aan te leggen.\\
...
Zoals beschreven in hoofdstuk \ref{ch:implementatie}, werd en een Python-toolbox ontwikkeld waarmee efficiënt de lenseigenschappen van een bepaald lensontwerp kunnen worden berekend. Deze toolbox maakt het mogelijk om de optimale lensparameters te bepalen via trial-and-error, door steeds de lensparameters aan te passen en de lenseigenschappen te berekenen. Het is echter nuttig om de invloed van de verschillende parameters te onderscheiden om ze individueel te kunnen onderzoeken. De toolbox wordt daarom uitgebreid om dit in kaart te brengen. Voorbeelden van \textit{variatieplots} die hierbij helpen worden getoond op Figuur \ref{fig:prop_grid_2x2} en \ref{fig:variable_T}.\\

Er wordt voor een bepaalde lensparameter een boven- en ondergrens gekozen waarvoor (voor vaste overige lensparameters) de lenseigenschappen voor een groot aantal punten tussen deze twee grenzen worden berekend. De resultaten kunnen op een grafiek worden weergegeven om zo nog efficiënter de lensparameters te optimaliseren. Voor vier verschillende lensparameters ($R_1$, $R_2$, $d_g$, en $B_r$) is deze methode relevant, en hiervoor werd een functie geïmplementeerd waarmee dit soort grafieken kunnen worden geplot. Naast de lensparameters wordt op deze grafieken ook de permeantiecoëfficiënt $P_{ci}$ geplot om te voorkomen dat de reluctantie van het magnetisch circuit te groot gemaakt wordt. Voor $B_r$ wordt dit niet gedaan aangezien $P_{ci}$ onveranderd blijft onder een herschaling van de $\mathrm{BH}$-curve. Het variëren van de de afmetingen van de magneet heeft enkle een verandering van $B_m$ als gevolg, en kunnen dus ook gemodelleerd worden door het variëren van $B_r$.\\
...
Er werden gelijkaardige variatieplots geïmplementeerd om ook de aberratiecoëfficiënten $D$ en $C_M$ in functie van de lensparameters $R_1$, $R_2$, $d_g$, en $B_r$ te bepalen. Grafieken van de aberratiecoëfficiënten in functie van de lensparameters worden getoond op Figuur \ref{fig:ab_grid_2x2}.
% Zoals eerder vermeld zijn $D$ en $C_M$ afhankelijk van de objectafstand $z_0$, dus om een correcte vergelijking tussen verschillende lensiegenschappen te maken, houden we de objectafstand constant op $z_0=25\mathrm{mm}$, wat een typicshe afstand is bij een focales lengtes waarbij we werken ()$f\sim 10\mathrm{mm}$). 
\begin{figure}[H]
    \centering
    ...
    \caption{Plots van aberratiecoëfficiënten $D$ en $C_M$ in functie van de geometrie van de lens bij $z_0=25\mathrm{mm}$. Deze plots geven weer hoe de parameters aangepast kunnen worden om de aberratiecoëfficiënten te minimaliseren.}
...
\end{tabular}
\caption{Gekozen lensparameters die een optimale lenswerking geven. De lens gebruikt een N35 graad NdfeB magneet, die een remanent veld van ongeveer $1.17\mathrm{T}$ heeft.}\label{tab:lens_parameters}
\end{table}
