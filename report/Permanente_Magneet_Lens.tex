...
\chapter{Methodologie simulatie lensparameters}\label{ch:methodologie}
...
\subsection{Kwalitatieve bespreking beeldvorming}
...
\subsubsection{Eerste experiment: EMAT logo}
...
\subsubsection{Tweede experiment: hexagonaal raster}
...
\subsubsection{Derde experiment: vierkant raster}
...
\end{enumerate}
Om dit te bereiken wordt de z-rail op zijn laagste positie gezet, en wordt de lens op een hogere stapel tussenringen geplaatst. Door de grotere afstanden tussen de componenten wordt er een hogere vergroting verwacht, dus wordt er gekozen voor een fijnmaziger raster met vierkante openingen van $\sim 55~\mu\mathrm{m}\times\sim 55~\mu\mathrm{m}$. Figuur \ref{fig:18.03.26_raster_Measurements} toont een SEM beeld van een opening in dit raster. In dit experiment werd gebruik gemaakt van de MiniPIX detector vanwege een probleem met de werking van de AdvaPIX detector.\\

Deze opstelling bleek een grote verbetering op de vorige experimenten te zijn. Figuur \ref{fig:18.03.26_raster} toont een MiniPIX-beeld van het raster waarbij er een groot aantal rasteropeningen te herkennen zijn. Door de grijswaarden te herschalen zodat lagere pixelwaarden ook zichtbaar worden werd Figuur \ref{fig:18.03.26_raster_Rescaled} verkregen, waardoor nog meer rasteropeningen zichtbaar worden. Éen rasteropening komt in werkelijkheid overeen met $\sim 55~\mu\mathrm{m}$, wat even groot is als 1 pixel op de detector. Voor de paraxiale elektronen zien we een uitvergroting van $\sim \times6$ aangezien deze rasteropeningen zoveel pixels op de detector bestrijken. De distortie blijft echter groot.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images/18.03.26_grid.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images/18.03.26_grid_Rescaled.png}
...
\subsection{Kwantitatieve validatie focale lengte}
...
\subsubsection{Mesh shadowgraph methode}
...
\subsubsection{Monte Carlo simulatie van het experiment}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Images/Monte_Carlo_2.png}
...
\subsubsection{3D-geprinte opstellingsonderdelen}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Images/3D_prints_measurements.jpeg}
    \caption{3D-geprinte opstellingsonderdelen gemaakt uit een geleidend PLA. Van links naar rechts: lenshouder, gridhouder, apertuur. Deze onderdelen zijn compatibel met ThorLabs $30$~mm cage plates.}
    \label{fig:Monte_Carlo_2}
\end{figure}
